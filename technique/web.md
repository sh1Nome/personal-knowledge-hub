...
* CNAMEレコード: ドメイン名を別のドメイン名にエイリアス（別名）として対応付ける。

#### ゾーンファイル
DNSサーバーが管理するドメインの情報を記述したテキストファイル。  
ゾーンファイルには、DNSレコードが記述される。

#### フルサービスリゾルバ
DNSサーバの分類のひとつ。  
キャッシュをもち、DNSキャッシュサーバとも呼ばれる。  
他のDNSサーバに問い合わせることもある。

#### 権威DNSサーバ
特定のドメインに対して正式な（権威のある）DNS情報を提供するサーバー。  
そのドメインに関する最終的な情報源となる。  
キャッシュではなく実際のゾーンファイルから情報を返す。
...
#### RFC Editor
RFCの発行と管理を行う組織。  
RFCの検索や閲覧ができるWebサイト（https://www.rfc-editor.org/）を提供している。

#### IETF Datatracker
IETF（Internet Engineering Task Force）のドキュメントやワーキンググループの活動を追跡するツール。  
インターネットドラフト（Internet-Draft）やRFCの進捗状況を確認できる（https://datatracker.ietf.org/）。  
RFCがどのような過程を経て標準化されたかを調べることができる。  
