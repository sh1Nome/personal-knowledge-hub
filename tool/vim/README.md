...
* 読み取り専用で起動 - `view {ファイル名}` or `vi -R {ファイルパス}`
* 標準入力を開く - `vim -`
...
### グローバルコマンド
* パターンにマッチした行でコマンドを実行 - `:g/pattern/cmd`
* パターンにマッチしない行でコマンドを実行 - `:g!/pattern/cmd`
...
### コメント
* 行をコメント - `gcc`
* ビジュアルモードで選択した範囲をコメント - `gc`

### テキスト整形
* テキストを左寄せ - `:left`
    * コマンドの後ろに数字を指定すると、その列数でインデントされる
* テキストを中央寄せ - `:center`
    * コマンドの後ろに数字を指定すると、その列数を基準に中央寄せされる
* テキストを右寄せ - `:right`
    * コマンドの後ろに数字を指定すると、その列数を基準に右寄せされる
...
* インサートモードで式を評価して挿入 - `<C-r>=`
    * 続けて式を入力してEnterキーで、その式の評価結果を挿入する
    * 例: `<C-r>=1+1<CR>`で、カーソル位置に`2`が挿入される
