...
#### git resetのオプション
`git reset`は指定したコミットにHEADを移動するコマンド。オプションによってステージング領域とワーキングディレクトリの扱いが異なる。

* `git reset --soft (コミット)`
    * 特定のコミットに移動し、移動したときの差分をステージング状態にする
    * コミットは取り消されるが、変更内容はステージングに残る
* `git reset --mixed (コミット)`
    * 特定のコミットに移動し、移動したときの差分のステージングを解除する（デフォルト動作）
    * コミットとステージングは取り消されるが、ワーキングディレクトリの変更は残る
* `git reset --hard (コミット)`
    * 特定のコミットに移動し、移動したときの差分の変更は削除する
    * コミット、ステージング、ワーキングディレクトリの変更すべてが失われる
...
#### git merge --squashの詳細な動作
`git merge --squash`は複数のコミットを1つにまとめてマージする。マージコミットは作成されず、変更はステージング状態になる。

動作例:
1. mainブランチから派生したxブランチにコミットa, bがある
2. mainブランチから派生したyブランチにコミットcがある
3. xブランチをmainブランチにマージ（通常のマージ）
4. mainブランチをyブランチに`git merge --squash main`でマージ

この場合:
* `git diff main..y`（2ドット）: mainとyの差分はcだけ
* `git diff main...y`（3ドット）: mainとyの共通祖先からyへの変更はa, b, cが表示される
* GitHubのプルリクエストに表示される差分は`git diff main...y`と同等なので、a, b, cが表示される

これは、squashマージによってマージコミットが作られず、履歴上はyブランチがmainブランチの変更を取り込んでいないように見えるため。  
3ドット記法は共通祖先を基準とするため、yブランチ側から見ると「まだマージされていない変更」としてa, bも含まれてしまう。
