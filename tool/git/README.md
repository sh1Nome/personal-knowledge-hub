...
#### git diffとgit logにおける2ドットと3ドットの違い
* `git diff A..B`: AとBの差分（2ドット）
    * `git diff A B`と同じ意味
* `git diff A...B`: AとBの共通の祖先とBの差分（3ドット）
* `git log A..B`: AからBへのコミット履歴（Aに含まれないBのコミット）
* `git log A...B`: AとBの共通の祖先から、AまたはBのどちらか一方のみに含まれるコミット

詳細は`git help diff`、`git help log`、`git help revisions`を参照
