...
* 現在のブランチ名のみを表示
    ...
    * スクリプトやコマンド実行時に現在のブランチ名を取得したい場合に便利
* 現在のブランチの名前を変更
    * `git branch -m (ブランチ名)`
* ブランチを強制削除
    * `git branch -D (ブランチ名)`
...
このとき`git show`は以下のようになる:
* `git show x..y`: yに含まれるが、xに含まれないコミットのみを表示（yのコミット）
* `git show x...y`: xまたはyのいずれかに含まれるが、両方には含まれないコミット両方を表示（xとyの両方のコミット）
...
* コミットメッセージで絞り込み
    * `git log --grep (検索文字列)`
* 指定した日時の範囲のコミットログを表示
    * `git log --since (日時) --until (日時)`
    * 日時フォーマット例: `2026-02-02`, `"2 weeks ago"`, `"3 days ago"`
...
* コミットログをグラフ表示
    * `git log --graph`
* すべての参照から辿れるコミットログを表示
    * `git log --all`
* 特定のパスのコミットログ
    * `git log -- (パス)`
* 特定のファイルのコミットログ（ファイルパスの変更も含めた）
...
このとき`git log`は以下のようになる:
* `git log x..y`: yに含まれるが、xに含まれないコミットのみを表示（yのコミット）
* `git log x...y`: xまたはyのいずれかに含まれるが、両方には含まれないコミット両方を表示（xとyの両方のコミット）
...
* サブモジュールを更新する
    * サブモジュールのディレクトリに移動してチェックアウト、親でコミット
...
* 初期ブランチをmainに指定しつつ初期化
    * `git init -b main`
* 問題のあるコミットを見つける（二分探索）
    * `git bisect start (悪いコミット) (良いコミット)`
        * 二分探索を開始し、指定した範囲内で問題のあるコミットを探す
    * `git bisect good`
        * 現在のコミットは良好状態（問題がない）
    * `git bisect bad`
        * 現在のコミットは問題がある
    * `git bisect run (コマンド)`
        * 指定したコマンドを実行し、戻り値で自動判定
        * 戻り値 0（成功） = good
        * 戻り値 0 以外（失敗） = bad
        * 戻り値 125 = skip（このコミットをスキップ）
    * `git bisect reset`
        * 二分探索を中止してリセット
    * `git bisect view`
        * 現在の二分探索の状態を表示
...
* ブランチ・タグは特定のコミットの参照
* ワーキングツリー
    * 作業ディレクトリのこと。実際にファイルを編集する場所
* インデックス
    * ステージングエリアのこと。コミット前にファイルの変更を一時的に保存する場所
